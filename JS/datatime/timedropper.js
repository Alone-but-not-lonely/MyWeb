(function ($) {
  $.fn.timeDropper = function (options, callbackFnk) {
    return $(this).each(function () {
      const
        _td_input = $(this)
      let _td_input_on = false
      let _td_mobile = false
      const _td_num = function (n) {
        return n < 10 ? '0' + n : n
      }
      const _td_id = $('.td-clock').length
      let _td_alert
      let _td_event = null
      const _td_options = $.extend({
      ...
      const _td_color = function (col, amt) {
        let usePound = false
      ...
      const _td_overlay = _td_container.find('.td-overlay')
      const _td_c = _td_container.find('.td-clock')
      ...
      let _td_h
      let _td_m
      const _td_define_deg = function () {
        const
          o = _td_c.find('.td-time span.on')
        const v = parseInt(o.attr('data-id'))
      ...
      const _td_rotation = function (deg) {
        const t = _td_c.find('.td-time span.on')

        let value = t.attr('data-id')

        if (!value) value = 0

        var h = Math.round((deg * 23 / 360))
        const m = Math.round((deg * 59 / 360))
        ...
        const _td_m = _td_c.find('.td-time span:last').attr('data-id')
      ...
      _td_c.find('.td-deg').on('touchstart mousedown', function (e) {
        ...
        const offset = _td_c.offset()
        const center = {
          y: offset.top + _td_c.height() / 2,
          x: offset.left + _td_c.width() / 2
        }

        let a; let b; let deg; let tmp; const rad2deg = 180 / Math.PI
      ...
      const _td_init = function (value) {
        const
          d = new Date()
        const _td_span_h = _td_c.find('.td-time span:first')
        const _td_span_m = _td_c.find('.td-time span:last')
        let h
        let m
