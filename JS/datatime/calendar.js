...
(function () {
  ...
  function renderMonth (year, month, clear, disabledDay, values) {
    const d = new Date(year, month - 1, 1); let s = "<div class='month-container'>" + '<table cellpadding="3" cellspacing="1" border="0"' +
				(clear ? ' class="right"' : '') +
				'>' +
    ...
    const firstDay = d.getDay(); let hit = false
    s += '<tr>'
    for (var i = 0; i < 7; i++) {
      if (firstDay == i || hit) {
        s += '<td' +
  ...
  function day_drap_listen () {
    ...
    $('.fullYearPicker .picker table td').mouseover(function () {
      const day = $(this).html()
      if (is_drap == 1 && day != '&nbsp;') {
        const min_date = getDateStr($(this)[0])
  ...
  function checkDate (start, end) {
    let rs = false
    const start_month = parseInt(start.split('-')[0])
    const start_day = parseInt(start.split('-')[1])
    const end_month = parseInt(end.split('-')[0])
    const end_day = parseInt(end.split('-')[1])
  ...
  $('#calendar_confirm_btn').click(function () {
    let start_date = $('#hd-start-date').val()
    start_date = start_date.split('-')[1] + '-' + start_date.split('-')[2]
    let end_date = $('#hd-end-date').val()
    end_date = end_date.split('-')[1] + '-' + end_date.split('-')[2]

    const m_type = $('#hd-type-option').val()
  ...
  function drap_select (start, end, new_class) {
    const max = 60// 当天数要选择到最后一天取一个大于所以月份的值
  ...
  $.fn.fullYearPicker = function (config, param) {
    ...
    return this
      ...
      .each(
        function () {
          const me = $(this); let year = config.year || new Date().getFullYear(); const newConifg = {
          ...
          me.append('<div class="year">' +
            ...
            .click(
              function (e, setYear) {
                if (setYear) { year = me.data('config').year } else { $(this).children('a').attr('class') == 'am-icon-chevron-left' ? year-- : year++ }
                setYearMenu(year)
                renderYear(
                  year,
